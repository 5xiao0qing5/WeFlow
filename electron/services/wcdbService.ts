...
export class WcdbService {
  ...
  private getDllPath(): string {
    ...
    if (this.resourcesPath && this.resourcesPath.length > 0) {
      return join(this.resourcesPath, 'wcdb_api.dll')
    }

    const fallbackBase = process.resourcesPath || join(process.cwd(), 'resources')
    return join(fallbackBase, 'wcdb_api.dll')
  }
